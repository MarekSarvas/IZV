...
def severity_wrt_cause(df, accidents, region='CZ', save_fig=None, show_fig=False):
    ...
    print('Počet usmrtených ľudí pri nehode spôsobenej vodičom {}'.format(cause_df[(cause_df['cause'] != 'nezavineny_vodicom') & (cause_df['severity'] == 'dead')].people.sum()))
    ...
    fig.set_figwidth(10)
    fig.set_figheight(6)
    fig.suptitle(f'Nasledky nehod v {region} sposobenych roznymi pricinami nehody')
    ...
    legend = ax.legend(h, labels, title="Zavaznost zraneni",loc='upper right', fontsize=10, frameon=False)
...
def accidents_cause(df, region='CZ', save_fig=None, show_fig=False):
    ...
    print('Pocet nehod zavinenych soferom motor. vozdila: {:2.4}%({})'.format(caused_by_df[caused_by_df.caused_by == 1].percent.tolist()[0], int(caused_by_df[caused_by_df.caused_by == 1].accidents.tolist()[0])))
    print('Pocet nehod zavinenych soferom nemotor. vozdila: {:2.4}%({})'.format(caused_by_df[caused_by_df.caused_by == 2].percent.tolist()[0], int(caused_by_df[caused_by_df.caused_by == 2].accidents.tolist()[0])))
    print('Pocet nehod zavinenych zverou: {:2.4}%({})'.format(caused_by_df[caused_by_df.caused_by == 3].percent.tolist()[0], int(caused_by_df[caused_by_df.caused_by == 3].accidents.tolist()[0])))
...
if __name__ == '__main__':
    ...
    df = clean_data(df)
    accidents = accidents_cause(df, region='JHM', show_fig=True)
